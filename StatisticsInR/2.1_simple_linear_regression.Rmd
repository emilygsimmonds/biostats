...
dive_data <- read_csv("../StatisticsInR/Files/dive_depths.csv", col_names=T)

# set the data type for each column
dive_data <- dive_data %>% mutate(max_depth = as.integer(max_depth), Species = 
                                  as.factor(species), Group = as.factor(group))

# load data for spurious correlation figure
Spurious <- read_csv("./Files/spurious.csv", col_names = TRUE)
...
your_data <- as_tibble(data.frame(response = 1:30,
                                 explanatory = rnorm(30,1:30,3)))

# create an example dataset for plotting
set.seed(2020) # make sure it is the same each time you run code
example_data <- as_tibble(data.frame(x = round(rnorm(100,1:100,10),2),
...
```{r plant-temp-fig, echo=FALSE, eval=TRUE, fig.cap="Panel A shows an illustration of inferring the effect of temperature on plant height. Panel B shows an illustration of the prediction of a plant height under a 2&deg;C increase in temperature. Created by Emily G. Simmonds", fig.height=150}
...
one could assume a perfect relationship (see \@ref(fig:fig-2)). 
...
uranium_plot <- ggplot(Spurious, aes(y=uranium_us, x=maths_phd))+
  ...
  labs(y="Uranium stored in US (million kg)", 
...
```{r figure-2, include=TRUE, echo=FALSE, fig.cap="Example of a spurious correlation (one that does not make sense causally). Scatterplot of number of maths PhDs awarded in a year against the amount of uranium stored in US power plants annually. Smooth line indicates a regression line for this data with the 95% confidence interval as the shaded area"}
...
```{r mammal-fig, echo=FALSE, eval=TRUE, fig.cap="Illustration of marine mammals by Emily G. Simmonds", fig.height=150}
...
```{r dive-fig, include=TRUE, echo=FALSE, fig.cap="Scatterplot of body size against maximum dive depth for marine mammals. Colours indicate taxonomic group"}
...
using the column names in place of `response` and 
`explanatory` and 
...
parameter_plot <- ggplot(your_data, aes(x=explanatory, y=response))+
  ...
  geom_abline(intercept=coef(model_object)[1], 
              slope=coef(model_object)[2], 
              color = "blue")+
  geom_point(aes(x=0, y=coef(model_object)[1]), 
                 colour = 'orange', size = 4, pch=8)+
  geom_segment(aes(x=13, xend=13, y=coef(model_object)[1]+(coef(model_object)[2]*12),
                                yend=coef(model_object)[1]+(coef(model_object)[2]*13)),
                colour = 'darkblue')+
  geom_segment(aes(x=12,xend=13, y=coef(model_object)[1]+(coef(model_object)[2]*12),
  ...
  geom_segment(aes(xend=explanatory, 
                   yend=fitted_values), colour = 'red')

```


```{r parameter-plot, include=TRUE, echo=FALSE, fig.cap="Illustration of components of a linear regression. Intercept is in orange, slope is in blue, and the residuals (explained later) are in red"}
...
These are highlighted red in \@ref(fig:parameter-plot).
...
and dive depth and as $X$ increases $Y$ decreases (\@ref(fig:fig-6)).
...
```{r dive-model-fig, include=TRUE, echo=FALSE, fig.cap="Scatter plot of dive depths against body size. The black line is the regression line predicted from the linear model"}
...
```{r dive-predictions-plot, include = TRUE, echo = FALSE, fig.cap="Scatter plot of dive depths against body size. Line is the regression line from a linear model. Shaded area is 95% confidence interval"}
...
```{r residuals-vs-fitted-fig, echo = FALSE, fig.cap="Example of a good (left) and a bad (right) residuals vs fitted plot for a simple linear regression"}
...
```{r normal-qq-fig, echo = FALSE, fig.cap="Example of a good (left) and a bad (right) normal QQ diagnostic plot"}
...
```{r scale-location-fig, echo = FALSE, fig.cap = "Example of a good (left) and a bad (right) scale-location plot"}
set.seed(2020)
...
```{r cook-d-fig, echo = FALSE, fig.cap = "Example of a good (left) and a bad (right) Cook's Distance plot"}
...
```{r dive-residual-fig, echo = TRUE, fig.cap = "Residuals vs fitted plot for dive depths model"}
...
\@ref(fig:dive-residual-fig) looks quite unusual. 
...
```{r dive-qq-fig, echo = TRUE, fig.cap="Normal QQ plot for dive depth model"}
...
These points also stood out in \@ref(fig:dive-residual-fig). 
We will need to look into them more in \@ref(fig:dive-cook-d-fig). 

</br>

#### Scale-location

```{r dive-scale-fig, echo = TRUE, fig.cap="Scale-location plot for dive depth model"}
...
\@ref(fig:fig-14) shows a very similar picture to \@ref(fig:fig-12). 
...
```{r dive-cook-d-fig, echo = TRUE, fig.cap = "Cook's distance plot for the dive depth model"}
...
\@ref(fig:fig-15) shows that the Cook's distances of this model are not 
very high (max = 0.2).
...
```{r final-dive-fig, include = TRUE, echo = FALSE, fig.cap = "Plot of the relationship between dive depth and body size faceted by taxonomic group"}
